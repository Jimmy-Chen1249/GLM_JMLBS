# Fixed tau = 0.15; rho.0 = sin(tau * pi / 2) = 0.00411
Gdata_Gau <- function(n, r, CR = c(4, 6), rho.0)
{
  set.seed(r)
  #*********Survival data first*********************
  #Gaussian Copula
  ...
  Temp.1 = (log(1 + k2 * (-log(1 - pnorm(T.star))[, 1]) / (k1 * exp(
    W %*% gamma.011 + beta.01 * b + gamma.012 * B11))) / k2)
  Temp.1[which(is.na(Temp.1) == 1)] = 0
  T.orig.1 = Temp.1 * (Temp.1 <= B13)
  
  Temp.2 = (log((k2 * (-log(1 - pnorm(T.star))[, 1]) / (k1 * exp(W %*% gamma.011 + beta.01 * b)) - exp(k2 * B13) * (exp(
  ...
  Temp.1 = (log(1 + k2 * (-log(1 - pnorm(T.star))[, 2]) / (k1 * exp(
    W %*% gamma.021 + beta.02 * b + gamma.022 * B21))) / k2)
  Temp.1[which(is.na(Temp.1) == 1)] = 0
  T.orig.2 = Temp.1 * (Temp.1 <= B23)
  
  Temp.2 = (log((k2 * (-log(1 - pnorm(T.star))[, 2]) / (k1 * exp(W %*% gamma.021 + beta.02 * b)) - exp(k2 * B23) * (exp(
...
Cen_Gau <- function(n, r, MR, rho.0)
{
  ...
  for(i in 1 : nrow(CRR))
  {
    dat = Gdata_Gau(n, r, CR = CRR[i, ], rho.0) 
    mr[i, ] = 1 - apply(dat$delta, 2, mean)
  }
  i.opt = which.min(apply(abs(mr - MR), 1, mean))
  return(Gdata_Gau(n, r, CR = c(CRR[i.opt, ]), rho.0))
}