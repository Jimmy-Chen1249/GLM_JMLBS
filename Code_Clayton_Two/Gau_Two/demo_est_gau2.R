...
nMC = 100; iter_max = 100; p = length(Theta.true)
...
for(r in 1:R)
{
  tempp = try(read.table(paste("Theta_Gaussian_two_n_", n, "_MR_", MR, "_rho_", rho.0, "_court_", r, ".txt", sep = "")), silent = TRUE)
...
Re = foreach(r = index,
             ...
             .export = c("n", "nMC", "iter_max", "p")) %dopar% {
               ...
               fileTheta = paste("Theta_Gaussian_two_n_", n, "_MR_", MR, "_rho_", rho.0, "_court_", r, ".txt", sep = "")
...
if(is.null(index1))
{
  for(r in (1:R))
  {
    filename1 = paste("Theta_Gaussian_two_n_", n, "_MR_", MR, "_rho_", rho.0, "_court_", r, ".txt", sep = "")
    tempp = unlist(read.table(filename1))
    Theta.hat = rbind(Theta.hat, tempp)
  }
}

fileTheta = paste("Theta_Gaussian_two_n_", n, "_MR_", MR, "_rho_", rho.0, "_R_", R, ".txt", sep = "")
