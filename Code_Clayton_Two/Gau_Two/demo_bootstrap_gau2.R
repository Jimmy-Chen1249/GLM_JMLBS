rm(list = ls())
setwd("~/Desktop/Chan-Ynu/Sci投稿与返修/CSDA/001_Data_CSDA_R2_2026-02-07/Clayton/Code_two/Gau_two_1/")
library(survival)
library(nlme)
library(lme4)
library(Rcpp)
...
library(parallel)
#library(ggplot2)
source("stepEM_Clay_Two.R")
source("mjoint_Clay_Two.R")
source("Gdata_Clay_Two.R")
source("initsSurv.R")
...
Theta.true <- c(as.numeric(alpha.0), sigma.0e, as.numeric(Sigma.b0), as.numeric(
...
fileTheta = paste("Theta_Gaussian_two_n_", n, "_MR_", MR, "_rho_", rho.0, "_R_", R, "_B_", B.boot, ".txt", sep = "")
fileSE = paste("SE_Gaussian_two_n_", n, "_MR_", MR, "_rho_", rho.0, "_R_", R, "_B_", B.boot, ".txt", sep = "")
fileCP = paste("CP_Gaussian_two_n_", n, "_MR_", MR, "_rho_", rho.0, "_R_", R, "_B_", B.boot, ".txt", sep = "")
#*************
t.start = Sys.time()
cl = makeCluster(getOption("cl.cores", 13))
envil = environment(Cen_Clay_Two)
registerDoSNOW(cl)
iterations = B.boot
...
for(r in c(1))
{
  ...
  Ree = try(mjoint_Clay_Two(Mixeffect, data = data, dat = dat, Sur1, Sur2, formSurv1, formSurv2,
                   nMC = nMC, iter_max = iter_max), silent = TRUE)
  if(!("try-error" %in% class(Ree)) && Ree$iter <= iter_max)
  {
    theta = Ree$theta
    MR.r = 1 - mean(apply(dat$delta, 2, mean))
    Theta.hatr = c(theta$alpha, sqrt(theta$sigma2), c(theta$Sigma.b), theta$gamma.1, theta$gamma.2, 
    ...
    Re = foreach(iter = 1:B.boot,
                 ...
                 .export = c("dat", "data", "n", "nMC", "iter_max",
                             "Mixeffect", "formSurv1", "formSurv2", "p")) %dopar% {
                               ...
                               formSurv1 = Surv(surtime, cens) ~ W
                               formSurv2 = Surv(surtime, cens) ~ W
                               fit_b = try(mjoint_Clay_Two(Mixeffect, data = data_b, dat = dat_b, Sur1_b, Sur2_b, formSurv1, formSurv2,
                                                  nMC = nMC, iter_max = iter_max), silent = TRUE)
                               ...
                               if (!inherits(fit_b, "try-error") && fit_b$iter <= iter_max) {
                                 ...
                                 Theta.hatbr = c(theta_b$alpha, sqrt(theta_b$sigma2), c(theta_b$Sigma),
