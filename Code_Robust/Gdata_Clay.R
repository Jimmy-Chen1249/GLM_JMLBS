# Fixed tau = 0.2; rho.0 = 2 * (1 / tau - 1) ^{-1} = 0.5
Gdata_Clay <- function(n, r, CR = c(4, 6), rho.0)
{
  set.seed(r)
  #*********Survival data first*********************
  #Clay Copula
...
Cen_Clay <- function(n, r, MR, rho.0)
{
  ...
  for(i in 1 : nrow(CRR))
  {
    dat = Gdata_Clay(n, r, CR = CRR[i, ], rho.0) 
    mr[i, ] = 1 - apply(dat$delta, 2, mean)
  }
  i.opt = which.min(apply(abs(mr - MR), 1, mean))
  return(Gdata_Clay(n, r, CR = c(CRR[i.opt, ]), rho.0))
}