####################Code_by Chan
rm(list = ls())
setwd("~/Desktop/Chan-Ynu/Sci投稿与返修/CSDA/001_Data_CSDA_R2_2026-02-07/Clayton/Code_one/Gau_one_1/")
library(survival)
library(nlme)
library(lme4)
library(Rcpp)
...
D.0 = 1; sigma.0e = 0.5; rho.0 = 0.3 #6/7

gamma.011 = as.matrix(0.3); gamma.012 = -0.3; beta.01 = 0.4
gamma.021 = as.matrix(0.4); gamma.022 = 0.3; beta.02 = 0.3

Theta.true <- c(as.numeric(alpha.0), sigma.0e, D.0, as.numeric(
  gamma.011), gamma.012, beta.01, as.numeric(gamma.021), gamma.022, beta.02, rho.0) #  rho.0 / (2 + rho.0))

#*****************************************************************************
n = 200    ##sample size
mtimes = 20   ##the maximum observed times
nMC = 100; MR = 0.3; iter_max = 100; B.boot = 100; p = length(Theta.true)
R = 200; R_tol = R; r.start = 1; Theta = NULL; SE = NULL; CP = NULL

fileTheta = paste("Theta_Gaussian_one_n_", n, "_MR_", MR, "_rho_", rho.0, "_R_", R, "_B_", B.boot, ".txt", sep = "")
fileSE = paste("SE_Gaussian_one_n_", n, "_MR_", MR, "_rho_", rho.0, "_R_", R, "_B_", B.boot, ".txt", sep = "")
fileCP = paste("CP_Gaussian_one_n_", n, "_MR_", MR,  "_rho_", rho.0, "_R_", R, "_B_", B.boot, ".txt", sep = "")
#*************
t.start = Sys.time()
cl = makeCluster(getOption("cl.cores", 13))
envil = environment(Cen_Clay)
registerDoSNOW(cl)
iterations = B.boot
...
for(r in 147:200)
{
  ...
  Ree = try(mjoint_Clay(Mixeffect, data = data, dat = dat, Sur1, Sur2, formSurv1, formSurv2,
  ...
  if(!("try-error" %in% class(Ree)) && Ree$iter <= iter_max)
  {
    ...
    Theta.hatr = c(theta$alpha, sqrt(theta$sigma2), sqrt(theta$D), theta$gamma.1, theta$gamma.2, 
                   theta$rho, Ree$iter, MR.r, r) # / (2 + theta$rho)
    ...
    Re = foreach(iter = 1:B.boot,
                 ...
                 .export = c("dat", "data", "n", "nMC", "iter_max",
                             "Mixeffect", "formSurv1", "formSurv2", "p")) %dopar% {
                               ...
                               formSurv1 = Surv(surtime, cens) ~ W
                               formSurv2 = Surv(surtime, cens) ~ W
                               fit_b = try(mjoint_Clay(Mixeffect, data = data_b, dat = dat_b, Sur1_b, Sur2_b, formSurv1, formSurv2,
                                                       nMC = nMC, iter_max = iter_max), silent = TRUE)
                               ...
                               if (!inherits(fit_b, "try-error") && fit_b$iter <= iter_max) {
                                 ...
                                 Theta.hatbr = c(theta_b$alpha, sqrt(theta_b$sigma2), sqrt(theta_b$D),
                                                 ...
                                                 theta_b$rho, fit_b$iter, MR.r_b, iter) # / (2 + theta_b$rho)
                               }
...
dat = read.table(fileCP, header = T)
CP_temp = dat[109:nrow(dat),]