...
mjoint_Clay_Two = function(Mixeffect, data, dat, Sur1, Sur2, formSurv1, formSurv2, nMC, iter_max)
{
  ...
  su1.para = initsSurv_one_t(data, Sur1, lfit, Sur1.1, "id", "time",  1)
  ...
  su2.para = initsSurv_one_t(data, Sur2, lfit, Sur2.1, "id", "time",  1)
  rm(W.2, Sur2, Sur2.1, Sur2.1.list, q.2, nev.2, nev.uniq.2, tj.2,
     Zdat.sur.2, Z.sur.2, Zt.sur.2, Zdat.2, Z.2, IW.2)
  
  #***********************************************************
  alpha0 = summary(lfit)$coefficient[, 1]
  sigma20 = summary(lfit)$sigma ^ 2
  ...
  haz.20 = su2.para$haz
  rho = 0.5; rho.region = c(0, 20);
  
  theta = list(alpha = alpha0, sigma2 = sigma20, Sigma.b = Sigma.b0, gamma.1 = gamma.10,
  ...
  while(tolerance>=tol & iter <= iter_max)
  {
    ...
    out = stepEM_Clay_Two(theta = theta, l = l, t.1 = t.1, s.1 = s.1,
                 t.2 = t.2, s.2 = s.2, nMC = nMC, rho.region = rho.region)
    theta.new = out$theta
    ...
    abssub = c(abs(alpha.new - alpha.old), abs(sigma2.new - sigma2.old),
               abs(as.numeric(Sigma.b.new - Sigma.b.old)), abs(gamma.1.new - gamma.1.old),
               abs(gamma.2.new - gamma.2.old), abs(rho.new - rho.old))
    rbssub = abssub / abs(c(alpha.old, sigma2.old, as.numeric(Sigma.b.old), gamma.1.old, gamma.2.old, rho.old))
    tolerance = min(max(abssub), max(rbssub))#max(max(abssub), max(c(abssub, ee1)))
    # print(rho.new)
    print(c(alpha.new, sigma2.new, c(Sigma.b.new), gamma.1.new, gamma.2.new, rho.new))
    iter = iter+1
    print(tolerance)
    ...
    if(tolerance < tol | iter > iter_max){message("EM algorithm has converged!\n")
      theta = theta.new
      break} else {
        nMC = min(nMC + floor(1 * nMC / 3), 2000)
        print(iter)
      }
