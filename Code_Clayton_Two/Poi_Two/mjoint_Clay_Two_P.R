...
mjoint_Clay_Two_P = function(Mixeffect, data, dat, Sur1, Sur2, formSurv1, formSurv2, nMC, iter_max)
{
  ...
  while(tolerance>=tol & iter <= iter_max)
  {
    ...
    res <- tryCatch({
      ...
      stepEM_Clay_Two_P(theta = theta, l = l, t.1 = t.1, s.1 = s.1,
    ...
    if (is.null(res) || elapsed > timeout_sec) {
      nMC = 100
      out <- stepEM_Clay_Two_P(theta = theta, l = l, t.1 = t.1, s.1 = s.1,
                    t.2 = t.2, s.2 = s.2, nMC = nMC, rho.region = rho.region)
    } else {
      out <- res
    }
