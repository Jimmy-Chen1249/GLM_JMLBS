stepEM_FGM = function(theta, l, t.1, s.1, t.2, s.2, nMC, rho.region)
{
  ...
  #**********************Pdf for Survival and longitudianl-FGM*************************************
  logf_sur = mapply(function(w.1, z.1, zsur.1, h.1, w.2, z.2, zsur.2, h.2) {
    ...
    if (h.1$delta == 1 && h.2$delta == 1) { # event
      (log(haz.1[ncol(zsur.1)]) + w.1 + log(z.1[ncol(zsur.1)]) + log(zsur.1[, ncol(zsur.1)])) - H.1 + (log(
        haz.2[ncol(zsur.2)]) + w.2 + log(z.2[ncol(zsur.2)]) + log(zsur.2[, ncol(zsur.2)])) - H.2 + log(rho * (
          2 * exp(-H.1) - 1) * (2 * exp(-H.2) - 1) + 1)
    }
    else if (h.1$delta == 1 && h.2$delta == 0) { # event
      (log(haz.1[ncol(zsur.1)]) + w.1 + log(z.1[ncol(zsur.1)]) + log(zsur.1[, ncol(zsur.1)])) - H.1 + log(exp(
        -H.2) + rho * exp(-H.2) * (1 - exp(-H.2)) * (1 - 2 * exp(-H.1)))
    }
    else if (h.1$delta == 0 && h.2$delta == 1) { # event
      (log(haz.2[ncol(zsur.2)]) + w.2 + log(z.2[ncol(zsur.2)]) + log(zsur.2[, ncol(zsur.2)])) - H.2 + log(exp(
        -H.1) + rho * exp(-H.1) * (1 - exp(-H.1)) * (1 - 2 * exp(-H.2)))
    }
    else if (h.1$delta == 0 && h.2$delta == 0) { # event
      log(exp(-H.1) * exp(-H.2) + rho * exp(-H.1) * exp(-H.2) * (1 - exp(-H.1)) * (1 - exp(-H.2)))
    }
  },
