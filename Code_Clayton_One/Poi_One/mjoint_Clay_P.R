...
mjoint_Clay_P = function(Mixeffect, data, dat, Sur1, Sur2, formSurv1, formSurv2, nMC, iter_max)
{
  #*********************Get initial value*****************************
  #****************Longitudinal first*******************
  ...
  #***********************************************************
  D20 <- as.numeric(summary(lfit)$varcor)
  ...
  haz.20 = su2.para$haz
  rho = 0.5; rho.region = c(1e-4, 30);
  theta = list(alpha = alpha0, D2 = D20, gamma.1 = gamma.10, rho = rho,
  ...
  {
    out = stepEM_Clay_P(theta = theta, l = l, t.1 = t.1, s.1 = s.1,
    ...
    abssub = c(abs(alpha.new - alpha.old),
               abs(D2.new - D2.old), abs(gamma.1.new - gamma.1.old),
               abs(gamma.2.new - gamma.2.old), abs(rho.new - rho.old))
    rbssub = abssub / abs(c(alpha.old, D2.old, gamma.1.old, gamma.2.old, rho.old))
