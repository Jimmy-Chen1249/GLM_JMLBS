####################Code_by Chan
rm(list = ls())
setwd("~/Desktop/Chan-Ynu/Sci投稿与返修/CSDA/001_Data_CSDA_R2_2026-02-07/Clayton/Code_one/Poi_one_1/")
library(survival)
library(nlme)
library(lme4)
library(Rcpp)
...
library(parallel)
#library(ggplot2)
source("stepEM_Clay_P.R")
source("mjoint_Clay_P.R")
source("Gdata_Clay_P.R")
...
Theta.true <- c(as.numeric(alpha.0), D.0, as.numeric(
  gamma.011), gamma.012, beta.01, as.numeric(gamma.021), gamma.022, beta.02, rho.0 / (2 + rho.0))

#*****************************************************************************
n = 300    ##sample size
mtimes = 20   ##the maximum observed times
nMC = 100; iter_max = 100; B.boot = 100; p = length(Theta.true)
R = 500; R_tol = R; r.start = 1; Theta = NULL; SE = NULL; CP = NULL

fileTheta = paste("Theta_Poisson_one_n_", n, "_MR_", MR, "_rho_", rho.0, "_R_", R, "_B_", B.boot, ".txt", sep = "")
fileSE = paste("SE_Poisson_one_n_", n, "_MR_", MR, "_rho_", rho.0, "_R_", R, "_B_", B.boot, ".txt", sep = "")
fileCP = paste("CP_Poisson_one_n_", n, "_MR_", MR, "_rho_", rho.0, "_R_", R, "_B_", B.boot, ".txt", sep = "")
#*************
t.start = Sys.time()
cl = makeCluster(getOption("cl.cores", 13))
envil = environment(Cen_Clay_P)
registerDoSNOW(cl)
iterations = B.boot
...
for(r in 1:500)
{
  ...
  Ree = try(mjoint_Clay_P(Mixeffect, data = data, dat = dat, Sur1, Sur2, formSurv1, formSurv2,
                   nMC = nMC, iter_max = iter_max), silent = TRUE)
  if(!("try-error" %in% class(Ree)) && Ree$iter <= iter_max)
  {
    theta = Ree$theta
    MR.r = 1 - mean(apply(dat$delta, 2, mean))
    Theta.hatr = c(theta$alpha, sqrt(theta$D), theta$gamma.1, theta$gamma.2, 
    ...
    Re = foreach(iter = 1:B.boot,
                 ...
                 .noexport = c("expWArma", "gammaUpdate", "hazHat", "uSamplerPoissonCpp_n", "lambdaUpdate", "gammaUpdate_fixDelta"),
                 .export = c("dat", "data", "n", "nMC", "iter_max",
                             "Mixeffect", "formSurv1", "formSurv2", "p")) %dopar% {
                               ...
                               formSurv1 = Surv(surtime, cens) ~ W
                               formSurv2 = Surv(surtime, cens) ~ W
                               fit_b = try(mjoint_Clay_P(Mixeffect, data = data_b, dat = dat_b, Sur1_b, Sur2_b, formSurv1, formSurv2,
                                                  nMC = nMC, iter_max = iter_max), silent = TRUE)
                               ...
                               if (!inherits(fit_b, "try-error") && fit_b$iter <= iter_max) {
                                 ...
                                 Theta.hatbr = c(theta_b$alpha, sqrt(theta_b$D),
